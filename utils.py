...
class PolicyAutoRegressiveModel(nn.Module):
    ...
    def forward(self, state):
        ...
        for j in range(self.num_dims):
            ...
            vals.append(self.undiscretize(sample, dimension=0)[:, None])
            # TODO end
    ...
    def log_prob(self, state, action):
        ...
        for j in range(self.num_dims):
            ...
            #pass
            inp = torch.cat([state, action[:, :j]], dim=-1)
            logits = self.trunks[j](inp)
            dist = Categorical(logits=logits)
            log_prob = log_prob + dist.log_prob(ac_discretized[:, j].long())
...
def rollout(
    ...
    while path_length < episode_length:
        o_for_agent = o.copy()
        o_for_agent = torch.from_numpy(o_for_agent[None].astype(np.float32)).to(device)
